...
export default class App extends Component{
  ...
  setOperation = operation => {
    if(this.state.current === 0){
      ...
      const values = [...this.state.values]
      try{
        values[0] = eval(`${values[0]} ${this.state.operation} ${values[1]}`)//verifica a op. recebida no display e executa no js
      }catch(e){
      ...
      this.setState({
        ...
        opreation: equals ? null: operation,
        current: equals ? 0 : 1,
        clearDisplay: true,
        values,
      })
